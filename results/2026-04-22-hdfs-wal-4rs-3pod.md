...
## 单 RegionServer 写入性能分析

基于 4 RS 均摊推算各维度单 RS 性能：

| 指标 | 数值 |
|---|---|
| 单 RS 全程平均 TPS | **~8,688 ops/sec** |
| 单 RS 调参后峰值 TPS | **~36,066 ops/sec** |
| 单 RS 平均写入速率（1KB/行） | **~8.5 MB/s** |
| 单 RS 峰值写入速率 | **~35.2 MB/s** |
| 单 RS 有效写入时间占比 | ~40~50%（flush 停顿期 TPS 归零）|

### 各轮测试单 RS 性能横向对比

| 测试轮次 | WAL 模式 | RS 数 | 全程平均单 RS TPS | 备注 |
|---|---|---|---|---|
| 2026-04-16 | EMR WAL | 4 | ~16,950 ops/sec | numgroups=4，10 Pod |
| 2026-04-22 上午 | HDFS WAL | 8 | ~5,941 ops/sec | 3 Pod |
| **2026-04-22 本轮** | **HDFS WAL** | **4** | **~8,688 ops/sec** | 3 Pod，含动态调参 |

> 📌 注意：各轮 Pod 数不同，单 RS TPS 受客户端压力影响，仅供参考趋势，不适合直接绝对值比较。
> EMR WAL 单 RS TPS 更高的原因是 WAL 异步写不阻塞写线程，flush 停顿占比更低（~30% vs ~50~60%）。
